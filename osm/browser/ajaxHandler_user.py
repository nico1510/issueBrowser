...
class AjaxHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):  
  ...
  def do_POST(self):    
    ...
    inBoundingBox = "user_domicile <@ box(point '(" + box[0] + "," + box[1] + ")', point '(" + box[2] + "," + box[3] + ")')"
    sqlMeldings = "select user_domicile, appuser_city from users where "+ inBoundingBox +";"
    print sqlMeldings
    cur.execute(sqlMeldings)
    resultObject = {}
    resultObject["meldings"] = cur.fetchall()

    sqlStats = "select hoofdcategorie_code, count(*) from melding group by hoofdcategorie_code"
