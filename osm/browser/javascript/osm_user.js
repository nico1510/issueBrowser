  ...
  $.ajax({
    ...
    success: function(data, status){
      for(var row in data.meldings){
        ...
        pos = data.meldings[row].user_domicile.substring(1,data.meldings[row].user_domicile.length-1).split(',');
        var m = L.marker([+pos[1],+pos[0]]).bindPopup("<b>"
                          + "</b><br>" + data.meldings[row].appuser_city,
                          {minWidth:250} );
        markers.addLayer(m);
      }
...
L.tileLayer('http://{s}.tiles.mapbox.com/v3/nico1510.ihb006cb/{z}/{x}/{y}.png', {
